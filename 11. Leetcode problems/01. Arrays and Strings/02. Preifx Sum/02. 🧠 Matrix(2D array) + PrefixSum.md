# 📌 PREFIX SUMS & MATRIX PROBLEMS — QUICK REVISION NOTES

---

## 1️⃣ Prefix Sum (1D) — Core Idea

**Definition**

```
prefix[i] = sum of elements from index 0 to i-1
prefix[0] = 0
```

**Subarray sum**

```
sum(j → i-1) = prefix[i] - prefix[j]
```

**Length of subarray**

```
length = i - j
```

🔑 `j` is the prefix **before** the subarray
🔑 Actual subarray = `(j+1 ... i)`

---

## 2️⃣ Why length is `i - j` (not i - (j-1))

* Prefix indices represent **gaps between elements**
* `j` is a **boundary**, not an element index
* Subarray starts at `j + 1`

```
| a | b | c | d |
0   1   2   3   4  ← prefix indices
```

Subarray between prefix 1 and 4:

```
length = 4 - 1 = 3
```

---

## 3️⃣ Prefix Sum Patterns (1D)

### Subarray Sum = K

```
prefix[i] - prefix[j] = k
prefix[j] = prefix[i] - k
```

### Subarray Divisible by K

```
(prefix[i] - prefix[j]) % k = 0
prefix[i] % k == prefix[j] % k
```

### Hashmap Rule

* Store **earliest index** → for max length
* Store **frequency** → for counting subarrays

---

## 4️⃣ Matrix Prefix Sum (Row-wise)

### Code

```python
for i in range(m):
    for j in range(1, n):
        matrix[i][j] += matrix[i][j-1]
```

### Meaning

```
matrix[i][j] = sum of row i from column 0 to j
```

Example:

```
Original row: [1, 2, 3, 4]
Prefix row:   [1, 3, 6, 10]
```

---

## 5️⃣ Why Row-wise Prefix Sum?

It allows **O(1)** row segment sums:

```
sum of row i from column c1 to c2 =
matrix[i][c2] - matrix[i][c1 - 1]
```

If `c1 == 0`:

```
sum = matrix[i][c2]
```

---

## 6️⃣ Key Line Explained

```python
row_sum = matrix[row][c2] - matrix[row][c1 - 1]
```

Meaning:

> Sum of elements in **one row**, between columns `c1` and `c2` (inclusive)

This is the **bridge** from 2D → 1D.

---

## 7️⃣ Matrix → 1D Compression Pattern (VERY IMPORTANT)

### Steps

1. Fix two columns `c1`, `c2`
2. For each row:

   ```
   row_sum[row] = sum of elements between c1 and c2
   ```
3. Now you have a **1D array**
4. Apply **prefix sum + hashmap**

💡 This is how **1074** works.

---

## 8️⃣ Full Pattern for Submatrix Sum Problems

```
2D matrix
   ↓
Row-wise prefix sum
   ↓
Fix column pairs (c1, c2)
   ↓
Compress rows → 1D array
   ↓
Prefix sum + hashmap
```

---

## 9️⃣ Time Complexity Cheat Sheet

| Problem Type          | Time       |
| --------------------- | ---------- |
| Brute force submatrix | O(m² · n²) |
| Row compression       | O(n² · m)  |
| Prefix + hashmap      | O(n)       |

---

## 🔟 Common Confusions (Cleared)

### ❌ Why not i - (j - 1)?

Because:

* `j` is **prefix index**
* Subarray starts at `j + 1`
* Length = `i - j`

### ❌ Why subtract `c1 - 1`?

Because:

* `matrix[row][c2]` includes everything from column 0
* Subtract unwanted left part

---

## 🧠 Memory Hooks (Interview Gold)

* **Prefix sum = boundary between elements**
* **Same prefix → zero difference**
* **Same remainder → divisible**
* **Matrix problems = compress → solve as 1D**
* **j is before subarray, not inside it**

---

## 🧾 Problems Covered by This Pattern

✔ 525 – Contiguous Array
✔ 560 – Subarray Sum Equals K
✔ 930 – Binary Subarrays With Sum
✔ 974 – Subarray Sums Divisible by K
✔ 1074 – Number of Submatrices That Sum to Target

---

## ✅ One-line Master Takeaway

> **Matrix subarray problems are just 1D prefix-sum problems in disguise.**

---